    ...
    <div class="row ">
        ...
        @foreach ($governs as $govern)
            <div class="col-md-4 col-sm-6 col-xs-12">
                <div class="info-box bg-{{ $govern->color }}">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{ $govern->govern_areas->count() }}</span>

                        <div class="progress">
                            <div class="progress-bar"
                                style="width:   {{ round($govern->govern_areas->count() / $total, 2) * 100 }}%"></div>
                        </div>
                        <span class="progress-description">
                            {{ round($govern->govern_areas->count() / $total, 3) * 100 }} % of total
                        </span>
                    </div>
                    <!-- /.info-box-content -->
                </div>
    ...
    <div class="row ">
        <div class="col-md-12 ">
            <div class="box" style="border: 0 !important;box-shadow: 0 1px 9px 3px rgba(0,0,0,0.1);">
                <div class="box-header">
                    <h3 class="box-title">{{ trans('areasTrans.CreateAreaTitle') }} </h3>
                </div>
                <div class="box-body ">
                    <div class="form-row">
                        ...
                        <div class="col-md-6">
                            ...
                            <button data-toggle="modal" data-target="#AddNewArea" class="btn btn-success">{{ trans('areasTrans.CreateArea') }} </button>
                        </div>
        ...
        <div class="row ">
            <div class="col-md-12 ">
                <div class="box" style="border: 0 !important;box-shadow: 0 1px 9px 3px rgba(0,0,0,0.1);">
                    <div class="box-header">
                        <h3 class="box-title">{{ trans('areasTrans.AreaList') }} </h3>
                    </div>
                    <div class="box-body ">
                        <div class="form-row">
                            <div class="table-responsitve">
                                <table id="TableAreas" class="table table-dark table-bordered  table-striped ">
                                    <thead>
                                        <tr>
                                            {{-- <th>#</th> --}}
                                            <th>{{ trans('areasTrans.AreaName') }}</th>
                                            <th></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($governs as $govern)
                                            <tr>
                                                @foreach ($govern->govern_areas as $area)
                                                    {{-- <td>
                                                        @if (App::islocale('ar'))
                                                            {{ $govern->arname }}
                                                        @else
                                                    ...
                                                    <td style=" text-align: end" class="text-end">
                                                        ...
                                                        <button onclick="setModalAttr(this)"  type="button" id="EditBtn"
                                                            AreaUUID="{{ $area->uuid }}"
                                                            AreaAr="{{ $area->arname }}"AreaName="{{ $area->name }}" governUUID="{{ $govern->uuid }}" 
                ...
                <form action="">
                    ...
                    <div class="modal-content">
                        <div class="modal-header bg-danger">
                            <h5 class="modal-title" id="exampleModalLongTitle">{{ trans('areasTrans.DeleteAreaTitle') }}</h5>
                        ...
                        <div class="modal-body">
                            <div class="form-row">
                                @csrf
                                {{ trans('areasTrans.DeleteAreaMessage') }}
                            </div>
                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group select">
                                            ...
                                            <select class="form-control select2 " style="width: 100%;" name=""
                                                id="EditAreaSelect">
                                                <option value="">{{ trans('areasTrans.selectChoose') }}</option>

                                                @foreach ($governs as $govern)
                                                    @if (App::islocale('ar'))
                                                        <option value="{{ $govern->uuid }}">{{ $govern->arname }}
                                                        </option>
                                                    @else
