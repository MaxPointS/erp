...
var Core = {
        ...
        pageTransition: function() {
            if($bodyWrapper.length) {
                $bodyWrapper.animsition({
                    ...
                    inDuration: 800,
                    outDuration: 800,
                    linkElement: 'a:not([target="_blank"]):not([href^="#"]):not([data-transit="0"]):not([href^="javascript:void(0)"])',
    ...
    Component: {
        ...
        forms: function(){
            ...
            // Contact Form
            var $registerForm  = $('#register-user');
            ...
            if($registerForm.length>0) {
                ...
                $registerForm.submit(function() {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if(data.status != 'success'){
                                    ...
                                    $btn.addClass('success');
                                    // window.location.replace('otp_confirmation');
                                    window.location.replace('http://al-kadi.grablugmah.com');
                                }
                            },
                            complete: function(data) {
                                setTimeout(function(){
                    ...
                    // else{
                    //     toastr.error('This field value is incorrect or empty');
                    // }
                    return false;
                });
...
function setStickyElement(scrolled) {
    ...
    if (scrolled + 150 > ($footer.offset().top - $stickableNav.outerHeight())) {
        $stickableNav.css({
            'position': 'absolute',
